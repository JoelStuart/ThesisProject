...
function [r,v,errork] =  kepler  ( ro,vo, dtseco, fid );
    ...
    show = 'n';
        ...
        if ( abs( dtseco ) > small )
            ...
            % -----------------  circle and ellipse -------------------
            if ( alpha >= small )
                period= twopi * sqrt( abs(a)^3.0/muV );
...
%                    dtsec = rem( dtseco,period );
                ...
                if ( abs(alpha-1.0 ) > small )
                     xold = sqrt(muV)*dtsec * alpha;
                  else
                     % - first guess can't be too close. ie a circle, r=a
                     xold = sqrt(muV) * dtsec * alpha * 0.97;
                  end
              else
                % --------------------  parabola  ---------------------
                if ( abs( alpha ) < small )
                    ...
                    magh = mag(h);
                    p= magh*magh/muV;
                    s= 0.5  * (halfpi - datan( 3.0 *sqrt( muV/ (p*p*p) )* dtsec ) );
            ...
            while ((abs(dtnew/sqrt(muV) - dtsec) >= small) & (ktr < numiter))
                xoldsqrd = xold*xold;
                znew     = xoldsqrd * alpha;
                ...
                rval = xoldsqrd*c2new + rdotv/sqrt(muV) *xold*(1.0 -znew*c3new) + ...
                         magro*( 1.0  - znew*c2new );
                dtnew= xoldsqrd*xold*c3new + rdotv/sqrt(muV)*xoldsqrd*c2new + ...
                         magro*xold*( 1.0  - znew*c3new );

                % ------------- calculate new value for x -------------
                xnew = xold + ( dtsec*sqrt(muV) - dtnew ) / rval;
...
%                 end
               ...
               if show =='y'
                   ...
                   fprintf(1,'%3i %11.7f %11.7f %11.7f %11.7f %11.7f \n', ...
                          ktr,xold/sqrt(re),znew,rval/re,xnew/sqrt(re),dtnew/sqrt(muV));
                 end
            ...
            if ( ktr >= numiter )
                ...
                g = dtsec - xnewsqrd*xnew*c3new/sqrt(muV);
                ...
                fdot = ( sqrt(muV)*xnew / ( magro*magr ) ) * ( znew*c3new-1.0  );
                for i= 1 : 3
